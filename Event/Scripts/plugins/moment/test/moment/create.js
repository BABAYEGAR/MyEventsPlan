...
test("array",
    function(assert) {
        ...
        assert.equal(+moment(new Date(2010, 1, 14, 15, 25, 50, 125)),
    ...
    function(assert) {
        var fmt = "YYYY-MM-DD HH:mm:ss.SSS",
            tests = [
                ...
                [
                    { years: 2010, months: 1, days: 14, hours: 15, minutes: 25, seconds: 50, milliseconds: 125 },
                    "2010-02-14 15:25:50.125"
                ],
                [
                    { year: 2010, month: 1, day: 14, hour: 15, minute: 25, second: 50, millisecond: 125 },
                    "2010-02-14 15:25:50.125"
                ],
...
test("unix",
    function(assert) {
        ...
        assert.equal(moment(1333129333524).unix(),
            1333129333,
            "1333129333524 Date.valueOf == 1333129333 unix timestamp");
        assert.equal(moment(1333129333524000).unix(),
            1333129333524,
...
test("date",
    function(assert) {
        ...
        assert.equal(moment(new Date(2016, 0, 1), "YYYY-MM-DD").format("YYYY-MM-DD"),
            "2016-01-01",
...
test("string with format dropped am/pm bug",
    function(assert) {
        ...
        assert.equal(moment("05/1/2012 12:25:00", "MM/DD/YYYY h:m:s a").format("MM/DD/YYYY"),
            "05/01/2012",
            "should not break if am/pm is left off from the parsing tokens");
        assert.equal(moment("05/1/2012 12:25:00 am", "MM/DD/YYYY h:m:s a").format("MM/DD/YYYY"),
            "05/01/2012",
            "should not break if am/pm is left off from the parsing tokens");
        assert.equal(moment("05/1/2012 12:25:00 pm", "MM/DD/YYYY h:m:s a").format("MM/DD/YYYY"),
            "05/01/2012",
            "should not break if am/pm is left off from the parsing tokens");
...
test("matching am/pm",
    function(assert) {
        assert.equal(moment("2012-09-03T03:00PM", "YYYY-MM-DDThh:mmA").format("YYYY-MM-DDThh:mmA"),
            "2012-09-03T03:00PM",
            "am/pm should parse correctly for PM");
        assert.equal(moment("2012-09-03T03:00P.M.", "YYYY-MM-DDThh:mmA").format("YYYY-MM-DDThh:mmA"),
            "2012-09-03T03:00PM",
            "am/pm should parse correctly for P.M.");
        assert.equal(moment("2012-09-03T03:00P", "YYYY-MM-DDThh:mmA").format("YYYY-MM-DDThh:mmA"),
            "2012-09-03T03:00PM",
        ...
        assert.equal(moment("2012-09-03T03:00AM", "YYYY-MM-DDThh:mmA").format("YYYY-MM-DDThh:mmA"),
            "2012-09-03T03:00AM",
            "am/pm should parse correctly for AM");
        assert.equal(moment("2012-09-03T03:00A.M.", "YYYY-MM-DDThh:mmA").format("YYYY-MM-DDThh:mmA"),
            "2012-09-03T03:00AM",
            "am/pm should parse correctly for A.M.");
        assert.equal(moment("2012-09-03T03:00A", "YYYY-MM-DDThh:mmA").format("YYYY-MM-DDThh:mmA"),
            "2012-09-03T03:00AM",
        ...
        assert.equal(moment("5:00p.m.March 4 2012", "h:mmAMMMM D YYYY").format("YYYY-MM-DDThh:mmA"),
            "2012-03-04T05:00PM",
...
test("unix timestamp format",
    function(assert) {
        ...
        for (i = 0; i < formats.length; i++) {
            format = formats[i];
            assert.equal(moment("1234567890", format).valueOf(),
                1234567890 * 1000,
                format + " matches timestamp without milliseconds");
            assert.equal(moment("1234567890.1", format).valueOf(),
                1234567890 * 1000 + 100,
                format + " matches timestamp with deciseconds");
            assert.equal(moment("1234567890.12", format).valueOf(),
                1234567890 * 1000 + 120,
                format + " matches timestamp with centiseconds");
            assert.equal(moment("1234567890.123", format).valueOf(),
                1234567890 * 1000 + 123,
...
test("string with timezone around start of year",
    function(assert) {
        assert.equal(moment("2000-01-01T00:00:00.000+01:00").toISOString(),
            "1999-12-31T23:00:00.000Z",
            "+1:00 around 2000");
        assert.equal(moment("2000-01-01T00:00:00.000-01:00").toISOString(),
            "2000-01-01T01:00:00.000Z",
            "-1:00 around 2000");
        assert.equal(moment("1970-01-01T00:00:00.000+01:00").toISOString(),
            "1969-12-31T23:00:00.000Z",
            "+1:00 around 1970");
        assert.equal(moment("1970-01-01T00:00:00.000-01:00").toISOString(),
            "1970-01-01T01:00:00.000Z",
            "-1:00 around 1970");
        assert.equal(moment("1200-01-01T00:00:00.000+01:00").toISOString(),
...
test("string with array of formats",
    function(assert) {
        assert.equal(moment("11-02-1999", ["MM-DD-YYYY", "DD-MM-YYYY"]).format("MM DD YYYY"),
            "11 02 1999",
            "switching month and day");
        assert.equal(moment("02-11-1999", ["MM/DD/YYYY", "YYYY MM DD", "MM-DD-YYYY"]).format("MM DD YYYY"),
            "02 11 1999",
            "year last");
        assert.equal(moment("1999-02-11", ["MM/DD/YYYY", "YYYY MM DD", "MM-DD-YYYY"]).format("MM DD YYYY"),
            "02 11 1999",
            "year first");

        assert
            .equal(moment("02-11-1999", ["MM/DD/YYYY", "YYYY MM DD"]).format("MM DD YYYY"), "02 11 1999", "year last");
        assert.equal(moment("1999-02-11", ["MM/DD/YYYY", "YYYY MM DD"]).format("MM DD YYYY"),
            "02 11 1999",
            "year first");
        assert
            .equal(moment("02-11-1999", ["YYYY MM DD", "MM/DD/YYYY"]).format("MM DD YYYY"), "02 11 1999", "year last");
        assert.equal(moment("1999-02-11", ["YYYY MM DD", "MM/DD/YYYY"]).format("MM DD YYYY"),
            "02 11 1999",
            "year first");

        assert.equal(moment("13-11-1999", ["MM/DD/YYYY", "DD/MM/YYYY"]).format("MM DD YYYY"),
            "11 13 1999",
            "second must be month");
        assert.equal(moment("11-13-1999", ["MM/DD/YYYY", "DD/MM/YYYY"]).format("MM DD YYYY"),
            "11 13 1999",
            "first must be month");
        assert.equal(moment("01-02-2000", ["MM/DD/YYYY", "DD/MM/YYYY"]).format("MM DD YYYY"),
            "01 02 2000",
            "either can be a month, month first format");
        assert.equal(moment("02-01-2000", ["DD/MM/YYYY", "MM/DD/YYYY"]).format("MM DD YYYY"),
            "01 02 2000",
            "either can be a month, day first format");

        assert.equal(moment("11-02-10", ["MM/DD/YY", "YY MM DD", "DD-MM-YY"]).format("MM DD YYYY"),
            "02 11 2010",
            "all unparsed substrings have influence on format penalty");
        assert.equal(moment("11-02-10", ["MM-DD-YY HH:mm", "YY MM DD"]).format("MM DD YYYY"),
            "02 10 2011",
            "prefer formats without extra tokens");
        assert.equal(moment("11-02-10 junk", ["MM-DD-YY", "YY.MM.DD junk"]).format("MM DD YYYY"),
            "02 10 2011",
            "prefer formats that dont result in extra characters");
        assert.equal(moment("11-22-10", ["YY-MM-DD", "YY-DD-MM"]).format("MM DD YYYY"),
            "10 22 2011",
            "prefer valid results");

        assert.equal(moment("gibberish", ["YY-MM-DD", "YY-DD-MM"]).format("MM DD YYYY"),
            "Invalid date",
        ...
        assert.equal(moment("11-02-10", ["MM.DD.YY", "DD-MM-YY"]).format("MM DD YYYY"),
            "02 11 2010",
...
test("string with array of formats + ISO",
    function(assert) {
        ...
        assert.equal(moment("17:15", [moment.ISO_8601, "MM", "HH:mm", "YYYY"]).hour(),
            17,
            "iso: assert parse HH:mm (1)");
        assert.equal(moment("17:15", [moment.ISO_8601, "MM", "HH:mm", "YYYY"]).minutes(),
            15,
            "iso: assert parse HH:mm (2)");
        assert.equal(moment("06", [moment.ISO_8601, "MM", "HH:mm", "YYYY"]).month(), 6 - 1, "iso: assert parse MM");
        assert.equal(moment("2012-06-01", [moment.ISO_8601, "MM", "HH:mm", "YYYY"]).parsingFlags().iso,
            true,
            "iso: assert parse iso");
        assert.equal(moment("2014-05-05", [moment.ISO_8601, "YYYY-MM-DD"]).parsingFlags().iso,
            true,
            "iso: edge case array precedence iso");
        assert.equal(moment("2014-05-05", ["YYYY-MM-DD", moment.ISO_8601]).parsingFlags().iso,
            false,
...
test("cloning carrying over utc mode",
    function(assert) {
        assert.equal(moment().local().clone()._isUTC,
            false,
        ...
        assert.equal(moment(moment().local())._isUTC,
            false,
    ...
    function(assert) {
        ...
        for (i = 0; i < formats.length; i++) {
            ...
            assert.equal(moment(formats[i][0], moment.ISO_8601, true).format("YYYY-MM-DDTHH:mm:ss.SSSZ"),
                formats[i][1],
                "moment should be able to parse specified strict ISO " + formats[i][0]);
        }
    });
...
test("parsing ISO with Z",
    function(assert) {
        ...
        for (i = 0; i < formats.length; i++) {
            ...
            assert.equal(mom.format("YYYY-MM-DDTHH:mm:ss.SSS"),
                formats[i][1],
...
test("parsing iso with T",
    function(assert) {
        ...
        assert.equal(moment("2011-10-08T18:20:13.321")._f,
        ...
        assert.equal(moment("2011-10-08 18:20:13.321")._f,
...
test("parsing iso Z timezone",
    function(assert) {
        ...
        for (i = 0; i < formats.length; i++) {
            assert.equal(moment.utc(formats[i][0]).format("YYYY-MM-DDTHH:mm:ss.SSSZ"),
                formats[i][1],
                "moment should be able to parse ISO " + formats[i][0]);
        }
    });
...
test("parsing iso Z timezone into local",
    function(assert) {
        ...
        assert.equal(m.utc().format("YYYY-MM-DDTHH:mm:ss.SSS"),
...
test("parsing iso with more subsecond precision digits",
    function(assert) {
        assert.equal(moment.utc("2013-07-31T22:00:00.0000000Z").format(),
...
test("six digit years",
    function(assert) {
        ...
        assert.equal(moment("+270000-01-01", "YYYYY-MM-DD").toDate().getFullYear(), 270000, "parse AD +270,000");
        assert.equal(moment.utc("-270000-01-01", "YYYYY-MM-DD").toDate().getUTCFullYear(),
            -270000,
...
test("strict parsing",
    function(assert) {
        ...
        assert.equal(moment("05 30 2010", ["DD MM YYYY", "MM DD YYYY"], true).format("MM DD YYYY"),
            "05 30 2010",
            "array with bad date");
        assert.equal(moment("05 30 2010", ["", "MM DD YYYY"], true).format("MM DD YYYY"),
            "05 30 2010",
...
test("parsing into a locale",
    function(assert) {
        ...
        assert.equal(moment("2012 seven", "YYYY MMM", "parselocale").month(),
            6,
...
test("parsing week and weekday information",
    function(assert) {
        ...
        assert.equal(moment("1999-W37-4 3:30", "GGGG-[W]WW-E HH:mm").format("YYYY MM DD HH:mm"),
            "1999 09 16 03:30",
...
test("utc with array of formats",
    function(assert) {
        assert.equal(moment.utc("2014-01-01", ["YYYY-MM-DD", "YYYY-MM"]).format(),
...
test("parsing flags retain parsed date parts",
    function(assert) {
        ...
        assert.equal(b.parsingFlags().parsedDateParts[0],
            undefined,
...
test("parsing only meridiem results in invalid date",
    function(assert) {
        assert.ok(!moment("alkj", "hh:mm a").isValid(),
            "because an a token is used, a meridiem will be parsed but nothing else was so invalid");
        assert.ok(moment("02:30 p more extra stuff", "hh:mm a").isValid(),
            "because other tokens were parsed, date is valid");
        assert.ok(moment("1/1/2016 extra data", ["a", "M/D/YYYY"]).isValid(),
