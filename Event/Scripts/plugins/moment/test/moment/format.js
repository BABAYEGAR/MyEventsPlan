...
test("format escape brackets",
    function(assert) {
        ...
        assert.equal(b.format("[L LL LLL LLLL aLa]"),
            "L LL LLL LLLL aLa",
            "localized tokens with escaped localized tokens");
        assert.equal(b.format("[LLL] LLL"),
...
test("utcOffset sanity checks",
    function(assert) {
        assert.equal(moment().utcOffset() % 15,
            0,
            "utc offset should be a multiple of 15 (was " + moment().utcOffset() + ")");

        assert.equal(moment().utcOffset(),
...
test("default UTC format",
    function(assert) {
        ...
        assert.ok(isoRegex.exec(moment.utc().format()),
            "default UTC format (" + moment.utc().format() + ") should match ISO");
    });
...
test("iso week year formats",
    function(assert) {
        ...
        for (i in cases) {
            ...
            assert.equal(isoWeekYear.slice(2, 4),
                formatted2,
...
test("calendar day timezone",
    function(assert) {
        ...
        for (i = 0; i < zones.length; ++i) {
            ...
            assert.equal(moment(a).utcOffset(z).calendar(),
                "Yesterday at 11:59 PM",
                "Yesterday at 11:59 PM, not Today, or the wrong time, tz = " + z);
        }

        assert.equal(moment(b).utc().calendar(),
            "Yesterday at 11:59 PM",
            "Yesterday at 11:59 PM, not Today, or the wrong time");
        assert.equal(moment(c).local().calendar(),
            "Yesterday at 11:59 PM",
            "Yesterday at 11:59 PM, not Today, or the wrong time");
        assert.equal(moment(c).local().calendar(d),
...
test("quarter ordinal formats",
    function(assert) {
        ...
        assert
            .equal(moment([2000, 0, 2]).format("Qo [quarter] YYYY"), "1st quarter 2000", "Jan  2 2000 is 1st quarter");
    });
