...
test("parse meridiem",
    function(assert) {
        ...
        assert.ok(b.isSame(moment(b.format("h:mm:ss a"), "h:mm:ss a", "ss", true), "seconds"),
            b.format("h:mm:ss a") +
            " should be equal to " +
            moment(b.format("h:mm:ss a"), "h:mm:ss a", "ss", true).format("h:mm:ss a"));

        // test that a formatted moment having a meridiem string can be parsed with strict flag
        assert.ok(moment(b.format("h:mm:ss a"), "h:mm:ss a", "ss", true).isValid(),
...
test("now from now",
    function(assert) {
        assert
            .equal(moment().fromNow(), "wenteka nga emizuzwana lomcane", "now from now should display as in the past");
    });
