...
test("weekdays strict parsing",
    function(assert) {
        ...
        for (var i = 0; i < 7; ++i) {
            ...
            assert.equal(moment(enLocale.weekdaysShort(m.day(i), ""), "ddd", true).isValid(),
                true,
                "parse short weekday " + i);
            assert.equal(moment(enLocale.weekdaysMin(m.day(i), ""), "dd", true).isValid(),
                true,
                "parse min weekday " + i);

            // negative tests
            assert.equal(moment(enLocale.weekdaysMin(m.day(i), ""), "ddd", true).isValid(),
                false,
                "parse short weekday " + i);
            assert.equal(moment(enLocale.weekdaysShort(m.day(i), ""), "dd", true).isValid(),
                false,
                "parse min weekday " + i);
        }
    });