...
test("from",
    function(assert) {
        ...
        assert.equal(start.from(moment([2007, 1, 28]).add({ s: 44 }), true),
            "ஒரு சில விநாடிகள்",
            "44 விநாடிகள் = ஒரு சில விநாடிகள்");
        assert.equal(start.from(moment([2007, 1, 28]).add({ s: 45 }), true),
            "ஒரு நிமிடம்",
            "45 விநாடிகள் = ஒரு நிமிடம்");
        assert.equal(start.from(moment([2007, 1, 28]).add({ s: 89 }), true),
            "ஒரு நிமிடம்",
            "89 விநாடிகள் = ஒரு நிமிடம்");
        assert.equal(start.from(moment([2007, 1, 28]).add({ s: 90 }), true),
            "௨ நிமிடங்கள்",
            "90 விநாடிகள் = ௨ நிமிடங்கள்");
        assert.equal(start.from(moment([2007, 1, 28]).add({ m: 44 }), true),
            "௪௪ நிமிடங்கள்",
            "44 நிமிடங்கள் = 44 நிமிடங்கள்");
        assert.equal(start.from(moment([2007, 1, 28]).add({ m: 45 }), true),
            "ஒரு மணி நேரம்",
            "45 நிமிடங்கள் = ஒரு மணி நேரம்");
        assert.equal(start.from(moment([2007, 1, 28]).add({ m: 89 }), true),
            "ஒரு மணி நேரம்",
            "89 நிமிடங்கள் = ஒரு மணி நேரம்");
        assert.equal(start.from(moment([2007, 1, 28]).add({ m: 90 }), true),
            "௨ மணி நேரம்",
            "90 நிமிடங்கள் = ௨ மணி நேரம்");
        assert.equal(start.from(moment([2007, 1, 28]).add({ h: 5 }), true), "௫ மணி நேரம்", "5 மணி நேரம் = 5 மணி நேரம்");
        assert.equal(start.from(moment([2007, 1, 28]).add({ h: 21 }), true),
            "௨௧ மணி நேரம்",
