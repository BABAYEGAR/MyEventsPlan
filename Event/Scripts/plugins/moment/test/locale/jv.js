...
test("format week",
    function(assert) {
        var expected = "Minggu Min Mg_Senen Sen Sn_Seloso Sel Sl_Rebu Reb Rb_Kemis Kem Km_Jemuwah Jem Jm_Septu Sep Sp"
                .split("_"),
            i;
        for (i = 0; i < expected.length; i++) {
...
test("from",
    function(assert) {
        ...
        assert.equal(start
            .from(moment([2007, 1, 28]).add({ s: 45 }), true),
            "setunggal menit",
            "45 seconds = a minute");
        assert.equal(start
            .from(moment([2007, 1, 28]).add({ s: 89 }), true),
            "setunggal menit",
            "89 seconds = a minute");
