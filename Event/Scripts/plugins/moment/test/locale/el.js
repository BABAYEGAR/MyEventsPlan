...
test("parse meridiem",
    function(assert) {
        ...
        assert.ok(b.isSame(moment(b.format("h:mm:ss a"), "h:mm:ss a", "el", true), "seconds"),
            b.format("h:mm:ss a") +
            " should be equal to " +
            moment(b.format("h:mm:ss a"), "h:mm:ss a", "el", true).format("h:mm:ss a"));

        // test that a formatted moment having a meridiem string can be parsed with strict flag
        assert.ok(moment(b.format("h:mm:ss a"), "h:mm:ss a", "el", true).isValid(),
...
test("calendar last week",
    function(assert) {
        ...
        for (i = 2; i < 7; i++) {
            ...
            assert.equal(m.calendar(),
                m.format(dayString + " [" + (m.hours() % 12 === 1 ? "στη" : "στις") + "] LT"),
                "Today - " + i + " days current time");
