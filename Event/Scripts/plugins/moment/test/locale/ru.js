...
test("parse",
    function(assert) {
        ...
        function equalTestStrict(input, mmm, monthIndex) {
            assert.equal(moment(input, mmm, true).month(),
                monthIndex,
...
test("format month case",
    function(assert) {
        var months = {
            ...
            i;
        for (i = 0; i < 12; i++) {
            assert.equal(moment([2011, i, 1]).format("D MMMM"),
...
test("format month short case",
    function(assert) {
        var monthsShort = {
            ...
            i;
        for (i = 0; i < 12; i++) {
            assert.equal(moment([2011, i, 1]).format("D MMM"),
                "1 " + monthsShort.accusative[i],
                "1 " + monthsShort.accusative[i]);
            assert.equal(moment([2011, i, 1]).format("MMM"),
...
test("format month case with escaped symbols",
    function(assert) {
        var months = {
            ...
            i;
        for (i = 0; i < 12; i++) {
            assert.equal(moment([2013, i, 1]).format("D[] MMMM"),
                "1 " + months.accusative[i],
                "1 " + months.accusative[i]);
            assert.equal(moment([2013, i, 1]).format("[<i>]D[</i>] [<b>]MMMM[</b>]"),
                "<i>1</i> <b>" + months.accusative[i] + "</b>",
                "1 <b>" + months.accusative[i] + "</b>");
            assert.equal(moment([2013, i, 1]).format("D[-й день] MMMM"),
...
test("format month short case with escaped symbols",
    function(assert) {
        var monthsShort = {
            ...
            i;
        for (i = 0; i < 12; i++) {
            assert.equal(moment([2013, i, 1]).format("D[] MMM"),
                "1 " + monthsShort.accusative[i],
                "1 " + monthsShort.accusative[i]);
            assert.equal(moment([2013, i, 1]).format("[<i>]D[</i>] [<b>]MMM[</b>]"),
                "<i>1</i> <b>" + monthsShort.accusative[i] + "</b>",
                "1 <b>" + monthsShort.accusative[i] + "</b>");
            assert.equal(moment([2013, i, 1]).format("D[-й день] MMM"),
...
test("from",
    function(assert) {
        ...
        assert.equal(start
            .from(moment([2007, 1, 28]).add({ s: 44 }), true),
            "несколько секунд",
            "44 seconds = seconds");
